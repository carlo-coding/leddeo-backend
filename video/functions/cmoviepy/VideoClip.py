...
from moviepy.compat import DEVNULL, string_types
from moviepy.config import get_setting
from moviepy.decorators import (
    add_mask_if_none, apply_to_mask, convert_masks_to_RGB,
...
from moviepy.tools import (
    ...
    find_extension, is_string, subprocess_call
)

from moviepy.video.io.ffmpeg_writer import ffmpeg_write_video
from moviepy.video.io.gif_writers import (
    write_gif, write_gif_with_image_io, 
...
class VideoClip(Clip):
    ...
    def blit_on(self, picture, t):
        ...
        if isinstance(pos[0], str):
            wug = (wf - wi)
            D = {'left': wug * 0.1, 'center': wug / 2, 'right': wug - wug * 0.1}
            pos[0] = D[pos[0]]

        if isinstance(pos[1], str):
            yug = (hf - hi)
            D = {'top': yug * 0.1, 'center': yug / 2, 'bottom': yug - yug*0.1}
            pos[1] = D[pos[1]]
